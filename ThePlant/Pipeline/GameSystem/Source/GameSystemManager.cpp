...
#include "Pipeline/Rendering/Headers/SingleCameraManager.h"
...
namespace GameSystem {
  GameSystemManager::GameSystemManager(Ptr<State> parentState, Rendering::RenderManager &renderSystem, Input::InputManager &inputSystem)
    : ParentState(parentState), RObjectManager(&renderSystem), MaterialManager(&renderSystem), _cameraManager(MakeUnique<Rendering::SingleCameraManager>(&renderSystem)), RenderSystem(renderSystem), InputSystem(inputSystem)
  {
