...
template <typename ENUM, ENUM... VALUES> struct enum_meta;

template <typename ENUM>
struct enum_meta<ENUM>
{
    template <typename T>
    static bool constexpr is_value(T) { return false; }
...
#define NAME(X) OBSTRUCT(GET_NAME)(SPLIT X) // 'NAME((A)(= 1))' into 'GET_NAME((A), (= 1))'
#define GET_NAME(X, Y) EXPAND X // 'GET_NAME((A), (= 1))' into 'A'
#define DEFINITION(X) OBSTRUCT(GET_DEFINITION)(SPLIT X) // 'NAME((A)(= 1))' into 'GET_DEFINITION((A), (= 1))'
...
\
    template <typename T> \
    static bool constexpr is_value(T t) { return data::is_value<T>(t); } \
...
#define ENUM_VALUE_INDIRECT(i, ...) OBSTRUCT(ENUM_VALUE)(i, ARG_I(i, __VA_ARGS__))

#define ENUM_VALUE(i, VALUE) \
IF(i)(OBSTRUCT(COMMA), OBSTRUCT(EMPTY))() \
NAME(VALUE) DEFINITION(VALUE)

#define ENUM_META_VALUE_INDIRECT(i, TYPE, ...) OBSTRUCT(ENUM_META_VALUE)(i, TYPE, ARG_I(i, __VA_ARGS__))
...
#define ENUM_STRING_VALUE_INDIRECT(i, TYPE, ...) OBSTRUCT(ENUM_STRING_VALUE)(i, TYPE, ARG_I(i, __VA_ARGS__))
